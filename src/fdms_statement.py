...
def create_fdms_bill(
    data: FDMSStatementData,
) -> tuple[str | None, str | None, str | None]:
    """Create bill in QuickBooks for FDMS statement.

    Args:
        data: Parsed FDMS statement data

    Returns:
        Tuple of (bill_url, chargebacks_text, adjustments_text)
        bill_url is None if bill creation fails
    """
    ...
    # Generate doc number: FDMS-{store}-{YYYYMM}
    doc_number = f"FDMS-{data.store_number}-{data.statement_month.strftime('%Y%m')}"

    # Build bill lines - all using account 6210
    lines: list[list[Any]] = []

    if data.interchange_program_fees > 0:
        lines.append(
            [
                qb.wmc_account_ref(FDMS_EXPENSE_ACCOUNT),
                "Interchange/Program Fees",
                str(data.interchange_program_fees),
            ]
        )

    if data.service_charges > 0:
        lines.append(
            [
                qb.wmc_account_ref(FDMS_EXPENSE_ACCOUNT),
                "Service Charges",
                str(data.service_charges),
            ]
        )
    ...
    # Create the bill using sync_bill
    try:
        qb.sync_bill(
            supplier=supplier,
        ...
        # Note: Bill URL requires actual bill ID from QuickBooks response
        # For now, we return None and the frontend shows the doc number
        bill_url = None
        ...
        return bill_url, chargebacks_text, adjustments_text
